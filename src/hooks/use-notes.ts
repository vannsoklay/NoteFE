...
export function useNotes() {
  ...
  // Filter state - will be passed to API
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      const params: FilterParams = {
        search: (searchQuery.value ?? query.search) || undefined,
        sortBy: sortBy.value || query.sortBy,
      }
  ...
  const editingNote = computed(() => {
    return editingId.value ? notes.value.find((n) => n.id === editingId.value) as Note : null
  })

  return {
    notes,
    sortedNotes: notes, // Already sorted from API, no need for computed
    loading,
    error,
    editingId,
    editingNote,
